...
import { Pokemon, Team, GenerationKey, TeamMember } from '../types';
...
import PokemonLoadoutModal from '../components/PokemonLoadoutModal';

// This is a browser-only library, so we need to declare it for TypeScript
declare const domtoimage: any;

const TeamMemberCard: React.FC<{ member: TeamMember; onRemove: (id: string) => void; onEdit: (member: TeamMember) => void }> = ({ member, onRemove, onEdit }) => (
    <div className="bg-poke-gray-dark rounded-xl p-4 relative shadow-lg transition-all duration-300 hover:shadow-yellow-500/20 hover:-translate-y-1 group">
        <button 
            onClick={() => onRemove(member.instanceId)} 
        ...
        >
            <div className="w-16 h-16 bg-gray-800/50 rounded-full flex items-center justify-center flex-shrink-0">
                <img src={member.sprite} alt={member.name} className="w-16 h-16" />
            </div>
            <div className="flex-1 min-w-0">
                <h3 className="text-xl font-bold capitalize text-white truncate">{member.nickname || member.name}</h3>
        ...
        <div className="absolute inset-0 bg-black/50 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
            <span className="text-white font-bold">Edit Loadout</span>
        </div>
    </div>
);
...
const TeamBuilderPage: React.FC = () => {
  const { activeTeam, removePokemonFromTeam, addPokemonToTeam, updateTeamMember } = useTeam();
                          ...
                          {activeTeam.members.length === 0 ? (
                              ...
                              <div className="grid lg:grid-cols-5 gap-8 items-stretch">
                                  <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-6">
                                      {activeTeam.members.map(m => <TeamMemberCard key={m.instanceId} member={m} onRemove={removePokemonFromTeam} onEdit={setEditingMember} />)}
